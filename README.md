...
При самом первом коммите в Git может появиться ошибка с требованиями заполнить информацию о пользователе:
```bash
git config --global user.name <name>
git config --global user.email <mail>
```

Чтобы проверить информацию, можно выполнить:
```bash
git config --list
```
...
Если к локальному репозиторию уже привязан удалённый и нужно только сменить ссылку:
```bash
